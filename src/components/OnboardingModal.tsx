  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-300 overflow-y-auto">
      <div className="w-full max-w-md glass-modal border border-slate-300/80 rounded-3xl shadow-2xl overflow-hidden flex flex-col text-slate-900 max-h-[90vh] my-auto">
        {/* Header */}
        <div className="theme-hero-card p-8 text-center relative border-b border-slate-300/30">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight opacity-95">Welcome</h2>
          <p className="text-xs mt-1 font-medium opacity-75">A calm, personal space for your mental wellness.</p>
        </div>

        {/* Form Body */}
        <form onSubmit={handleSubmit} className="p-8 space-y-6 glass-modal border-none">
          <div className="space-y-2">
            <label htmlFor="user-name-input" className="block text-xs font-bold uppercase tracking-wider opacity-90">
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full pl-11 pr-4 py-3.5 glass-card border border-slate-300/40 rounded-2xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
                autoFocus
              />
              <User className="w-5 h-5 opacity-60 absolute left-3.5 top-3.5" />
            </div>
          </div>
          ...
          <p className="text-[11px] text-center opacity-75 font-medium">
